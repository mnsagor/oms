...
class UpdateHmRequest extends FormRequest
{
    public function authorize()
    {
        return Gate::allows('hm_edit');
    }
    ...
    public function rules()
    {
        return [
            'name' => [
                'string',
                'required',
                'unique:hms,name,' . request()->route('hm')->id,
            ],
            'username' => [
                'string',
                'required',
                'unique:hms,username,' . request()->route('hm')->id,
            ],
            'email' => [
                'required',
                'unique:hms,email,' . request()->route('hm')->id,
            ],
            'connection_date' => [
            ...
            'bill_send_mail' => [
                'required',
                'unique:hms,bill_send_mail,' . request()->route('hm')->id,
            ],
            'previous_company' => [
                'string',
                'nullable',
