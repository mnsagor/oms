    ...
    @can('visited_hospital_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                ...
                @include('csvImport.modal', ['model' => 'VisitedHospital', 'route' => 'admin.visited-hospitals.parseCsvImport'])
            </div>
        </div>
    @endcan
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-VisitedHospital">
                        <thead>
                            <tr>
                                <th width="10">
                            ...
                            <tr>
                                ...
                                <td>
                                    <select class="search" strict="true">
                                        ...
                                        @foreach(App\Models\VisitedHospital::STATUS_SELECT as $key => $item)
                                ...
                                <td>
                                    <select class="search" strict="true">
                                        <option value>{{ trans('global.all') }}</option>
                                        @foreach(App\Models\VisitedHospital::IS_ONLINE_SELECT as $key => $item)
                                            <option value="{{ $key }}">{{ $item }}</option>
                                        @endforeach
                                    </select>
                                </td>
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.visited-hospitals.index') }}",
    columns: [
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-VisitedHospital').DataTable(dtOverrideGlobals);
