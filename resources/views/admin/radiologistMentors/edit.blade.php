        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.radiologistMentor.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.radiologist-mentors.update", [$radiologistMentor->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.radiologistMentor.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $radiologistMentor->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('degree') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="degree" id="degree" value="{{ old('degree', $radiologistMentor->degree) }}">
                        ...
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="address" id="address">{!! old('address', $radiologistMentor->address) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('phone_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $radiologistMentor->phone_number) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $radiologistMentor->email) }}" required>
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('joining_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="joining_date" id="joining_date" value="{{ old('joining_date', $radiologistMentor->joining_date) }}">
                        ...
                        <div class="form-group {{ $errors->has('hospital_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="hospital_name_id" id="hospital_name_id">
                                @foreach($hospital_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('hospital_name_id') ? old('hospital_name_id') : $radiologistMentor->hospital_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('reporting_price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="reporting_price" id="reporting_price" value="{{ old('reporting_price', $radiologistMentor->reporting_price) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('payment_method') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="payment_method" id="payment_method" value="{{ old('payment_method', $radiologistMentor->payment_method) }}">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status" required>
                                ...
                                @foreach(App\Models\RadiologistMentor::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $radiologistMentor->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
